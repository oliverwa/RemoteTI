#!/usr/bin/env bash
# Simple camera fetch script: camera_fetch.sh <hangar> <drone> <camera_name> [session_timestamp] [port] [folder_name]
set -euo pipefail

if [ $# -lt 3 ] || [ $# -gt 6 ]; then
    echo "Usage: $0 <hangar> <drone> <camera_name> [session_timestamp] [port] [folder_name]"
    ...
    echo "Example: $0 hangar_sisjon_vpn bender FDR 251006_203000 8084 Molndal_hangar"
    exit 1
fi
...
SESSION_TIMESTAMP="${4:-}"
CUSTOM_PORT="${5:-}"
FOLDER_NAME="${6:-$HANGAR_HOST}"  # Use folder name if provided, otherwise hangar host
...
OUT_DIR="/home/pi/RemoteTI/data/sessions/${FOLDER_NAME}/${DRONE_NAME}_${RUN_STAMP}"
mkdir -p "${OUT_DIR}"
